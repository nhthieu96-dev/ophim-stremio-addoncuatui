...
// Ánh xạ path danh mục -> giá trị "type" thật của OPhim để lọc kết quả tìm
// kiếm đúng danh mục (API tìm kiếm trả về TẤT CẢ loại phim trộn lẫn, phải tự
// lọc lại theo type). OPhim dùng 4 type: single (phim lẻ), series (phim bộ),
// hoathinh (hoạt hình), tvshows (tv shows).
...
// Domain + thư mục ảnh CHUẨN đã xác nhận thực tế qua endpoint chính thức
// GET /phim/{slug} (trả về poster_url dạng URL đầy đủ như:
// "https://img.ophim.live/uploads/movies/kiem-vu-poster.jpg").
// KHÔNG dùng "pathImage" trả về từ endpoint danh sách không chính thức vì nó
// thiếu đoạn "/uploads/movies/", gây lỗi 404 khi tải ảnh.
...
function toImageUrl(value) {
  ...
  // Chỉ là filename -> ghép với domain ảnh chuẩn
...
function itemToMetaPreview(item) {
  return {
    ...
    name: item.name,
    poster: toImageUrl(item.poster_url || item.thumb_url),
...
builder.defineCatalogHandler(async ({ type, id, extra }) => {
  try {
    ...
    if (searchTerm) {
      ...
      // "phim-moi-cap-nhat" không lọc (hiện tất cả loại phim khi tìm kiếm ở đây)
      items = wantedType
